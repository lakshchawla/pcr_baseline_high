    ...
    for epoch in range(cfg.optim.epochs):
        ...
        for it, b_idx in enumerate(batches):
            ...
            if (it + 1) % cfg.logging.print_freq == 0:
                print('Epoch: [{}][{}/{}]\tLoss {:.3f}\tLR {:.2e}\tVAB gate {:.3f}\tPool gate {:.3f}'
                      '\trelalign {:.4f} (x{:.2f})'.format(
                    epoch, it + 1, iters_per_epoch, loss.item(), optimizer.param_groups[0]['lr'],
                    torch.tanh(vab.gate).item(), torch.tanh(pool.gate).item(),
                    l_relalign.item(), lambda_relalign))
